...
import type { ChatItem, MessageItem, ReportItem, ActionRequestItem, Jurisdiction, Evidence, Report, AiMode, FileInfo } from './types';
import { getGeminiResponse, generateSealedReportFromEvidence, getJurisdictionFromCoords, generateContradictionAnalysis } from './services/geminiService';
import { calculateSHA512, saveEvidence, saveReport, getAllReportsIndexed, clearEvidenceAndReports, getEvidenceById, setPdfJsWorkerSrc, extractPdfText } from './services/forensicService';
...
const App: React.FC = () => {
    ...
    const addChatItem = useCallback((item: Omit<MessageItem, 'id' | 'timestamp' | 'type'>) => {
        const newItem: MessageItem = { ...item, id: crypto.randomUUID(), timestamp: Date.now(), type: 'message' };
        ...
        try {
            ...
            if (reports.length === 0) {
                ...
                const loadedItems: ReportItem[] = reports.reduce<ReportItem[]>((acc, report) => {
                    const primaryEvidence = evidenceMap.get(report.evidenceRefs[0]?.id);
                    if (primaryEvidence) {
                        acc.push({
                            id: report.id,
                            type: 'report',
                ...
                setChatItems(loadedItems);
                 addChatItem({ sender: 'assistant', content: `Case context rebuilt from local storage. Jurisdiction: **${context.jurisdiction}**.` });
            }
        } catch (error) {
            console.error("Failed to load data from DB:", error);
            addChatItem({ sender: 'assistant', content: 'Could not load saved data. Starting a fresh session.' });
        } finally {
            setIsLoading(false);
        }
    }, [addChatItem]);
    ...
    // Effect to load data once location is granted or denied (with default)
    useEffect(() => {
        if (sessionContext) {
    ...
    const handleSendMessage = async (text: string) => {
        ...
        addChatItem({ sender: 'user', content: text, mode: aiMode, jurisdiction: sessionContext.jurisdiction });
        setIsLoading(true);
        ...
        try {
            ...
            addChatItem({ sender: 'assistant', content: response, mode: aiMode, jurisdiction: sessionContext.jurisdiction });
        } catch (error) {
            console.error(error);
            addChatItem({ sender: 'assistant', content: 'Sorry, I ran into an error.', mode: aiMode, jurisdiction: sessionContext.jurisdiction });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const runAnalysisAndSaveReport = useCallback(async (evidence: Evidence) => {
        ...
        try {
            // Triple verification simulation
            ...
            if (validResults.length === 0) {
                 addChatItem({ sender: 'assistant', content: "Analysis was inconclusive. The AI could not generate a valid report for this evidence. Please try another file."});
                 return;
            }
            ...
            addChatItem({ sender: 'assistant', content: `${primaryResult.intro || "Analysis complete."}\n\nReport saved with **${verificationLabel}** status.` });
            ...
            addChatItem({ sender: 'assistant', content: 'A critical error occurred during report generation.' });
        } finally {
            setIsLoading(false);
    ...
    const handleFileUpload = async (file: File) => {
        ...
        addChatItem({ sender: 'user', fileInfo, content: `File for analysis: ${file.name}` });
        addChatItem({ sender: 'assistant', content: `Got the file. Initiating forensic analysis now...\nFile fingerprint (SHA-512) computed for integrity verification.`});
        ...
        try {
            ...
            if (file.type === 'application/pdf') {
                ...
                if (text.trim().length < 100) {
                    const actionItem: ActionRequestItem = {
                        id: crypto.randomUUID(),
                        type: 'action_request',
                        timestamp: Date.now(),
                        content: "This PDF appears to be a scanned image with little to no text. How should I proceed?",
                        actions: [
                            { label: 'Analyze as Image', callback: () => runAnalysisAndSaveReport(evidence) }
                        ]
                    };
                    setChatItems(prev => [...prev, actionItem]);
                    setIsLoading(false);
                    setIsProcessingFile(false);
                    return;
                }
            ...
            await runAnalysisAndSaveReport(evidence);

        } catch (error) {
            console.error("File upload/processing failed:", error);
            addChatItem({ sender: 'assistant', content: 'Failed to save or analyze the evidence.' });
            setIsLoading(false);
            setIsProcessingFile(false);
        }
    };
    ...
    const handleAnalyzeContradictions = async () => {
        ...
        addChatItem({ sender: 'user', content: `Analyzing ${selectedEvidenceIds.size} pieces of evidence for contradictions.` });
        setIsLoading(true);
        
        try {
            const evidenceToAnalyze = await Promise.all(
                Array.from(selectedEvidenceIds).map(id => getEvidenceById(id))
            );
            const validEvidence = evidenceToAnalyze.filter(Boolean) as Evidence[];
            
            if (validEvidence.length < 2) {
                throw new Error("Could not retrieve sufficient evidence for analysis.");
            }
            
            const contradictions = await generateContradictionAnalysis(validEvidence, sessionContext.jurisdiction);
            
            const newReport = await saveReport({
                title: `Analysis of ${validEvidence.length} items`,
                jurisdiction: sessionContext.jurisdiction,
                timezone: sessionContext.timezone,
                evidenceRefs: validEvidence.map(e => ({ id: e.id, sha512: e.sha512 })),
                findings: [],
                contradictions: contradictions,
                timeline: [],
            });
            
            addChatItem({ sender: 'assistant', content: `Contradiction analysis complete. Found ${contradictions.length} potential contradictions. A new report has been generated.`});
            
            // Reload all data to show the new report
            if (sessionContext) {
...
// FIX: Safely handle 'unknown' error type in catch block.
            ...
            if (e instanceof Error) {
                console.error(`Failed to run contradiction analysis: ${e.message}`, e);
                addChatItem({sender: 'assistant', content: `Failed to run contradiction analysis: ${e.message}`});
            } else {
                // FIX: Safely handle 'unknown' error type by converting it to a string for logging.
                console.error(`An unexpected error occurred during contradiction analysis: ${String(e)}`);
                addChatItem({sender: 'assistant', content: 'An unexpected error occurred during contradiction analysis.'});
            }
        } finally {
    ...
    return (
        <div className="flex flex-col h-full bg-gray-950">
            ...
            <header className="flex items-center justify-between p-4 bg-gray-900/50 border-b border-gray-700 backdrop-blur-sm sticky top-0 z-10">
                <div className="flex items-center space-x-3">
                    <svg className="w-8 h-8 text-blue-400" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="48" fill="#1f2937" stroke="#4b5563" strokeWidth="2"/><path d="M 50,15 A 35,35 0 1 1 50,85" fill="none" stroke="#e5e7eb" strokeWidth="4" strokeDasharray="5 5" transform="rotate(45 50 50)"/><path d="M 50,25 A 25,25 0 1 0 50,75" fill="none" stroke="#6b7280" strokeWidth="3"/><circle cx="50" cy="50" r="10" fill="#3b82f6"/></svg>
                    <div>
                        <h1 className="text-xl font-bold text-gray-100">Verum Omnis</h1>
                ...
                <div className="flex items-center space-x-4">
                    ...
                    {selectedEvidenceIds.size > 1 && (
                        <button onClick={handleAnalyzeContradictions} className="px-3 py-1.5 text-sm font-semibold text-white bg-yellow-600 rounded-md hover:bg-yellow-700 transition">
                            Analyze Contradictions ({selectedEvidenceIds.size})
                        </button>
                    )}
                     <button onClick={() => setIsClearModalOpen(true)} className="px-3 py-1.5 text-sm font-semibold text-white bg-red-800 rounded-md hover:bg-red-700 transition">
                        Clear Case
                    </button>
                </div>
            </header>
