...
import type { FileInfo, AiMode, Jurisdiction, Finding, Highlight, Evidence, Contradiction } from '../types';
...
export const generateSealedReportFromEvidence = async (
    ...
    try {
        const imagePart = await fileToGenerativePart(evidence.blob);
        const prompt = `Analyze the attached document image. 
        ...
        Generate a sealed forensic report based on its content by following the SEALED REPORT GENERATION PROTOCOL exactly. Start with a brief confirmation message, then provide the JSON object.`;
        ...
        if (jsonMatch && jsonMatch[1]) {
            ...
            try {
                ...
                if (data.reportHtml && data.findings && data.highlights) {
...
export const generateContradictionAnalysis = async (
    evidenceList: Evidence[],
    jurisdiction: Jurisdiction
): Promise<Contradiction[]> => {
     try {
        const evidenceSummary = evidenceList.map(e => `- Evidence ID: ${e.id}, Name: ${e.name}, Type: ${e.type}, SHA512: ${e.sha512}`).join('\n');
        
        const prompt = `
        You are the Contradiction Engine for Verum Omnis. Analyze the following set of evidence summaries for contradictions.
        Jurisdiction: ${jurisdiction}
        Evidence Set:
        ${evidenceSummary}

        Identify contradictions of the following types:
        - direct: The same actor or entity makes mutually exclusive claims across different documents.
        - metadata_mismatch: Timestamps, authors, or other metadata in file headers (EXIF, PDF metadata) conflict with claims made within the document content.
        - cross_doc_drift: Key identifiers, names, or amounts change slightly or significantly across documents without explanation.
        - omission: A document references an exhibit, attachment, or another piece of evidence that has not been provided in this set.
        
        For each contradiction found, provide a structured entry in a JSON array. If no contradictions are found, return an empty array.
        `;
        ...
        const response = await ai.models.generateContent({
            ...
            contents: prompt,
            config: {
                systemInstruction: `You are a deterministic contradiction engine. Your output MUST be a valid JSON array conforming to the specified schema. Do not add any explanatory text outside the JSON structure.`,
                ...
                responseSchema: {
                    ...
                    items: {
                        type: Type.OBJECT,
                        properties: {
                            type: { type: Type.STRING, description: "One of: 'direct', 'metadata_mismatch', 'cross_doc_drift', 'omission'" },
                            actor: { type: Type.STRING, description: "The person or entity involved, if applicable." },
                            claimA: { type: Type.STRING, description: "The first conflicting claim or piece of data." },
                            claimB: { type: Type.STRING, description: "The second conflicting claim or piece of data." },
                            sources: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Array of Evidence IDs involved in the contradiction." },
                            explanation: { type: Type.STRING, description: "A brief, neutral explanation of the contradiction." },
                        },
                        required: ['type', 'sources', 'explanation'],
                    },
                },
            },
        });
