...
const ContradictionDisplay: React.FC<{ contradiction: Contradiction }> = ({ contradiction }) => {
    const verificationColorClasses: Record<Contradiction['verification'], string> = {
        'Verified (3/3)': 'bg-green-600 text-green-200 ring-green-500/50',
        'Consensus (2/3)': 'bg-yellow-600 text-yellow-200 ring-yellow-500/50',
        'Inconclusive (≤1/3)': 'bg-gray-600 text-gray-300 ring-gray-500/50'
    };

    return (
         <div className="p-3 rounded-lg bg-red-900/20 ring-1 ring-red-500/30 space-y-2">
            <div className="flex justify-between items-start">
                <h5 className="font-semibold text-base text-red-300 capitalize">Contradiction: {contradiction.type.replace(/_/g, ' ')}</h5>
                 <span className={`px-2 py-0.5 text-xs font-bold rounded-full ring-1 ${verificationColorClasses[contradiction.verification]}`}>
                    {contradiction.verification}
                </span>
            </div>
            <p className="text-sm"><strong>Explanation:</strong> {contradiction.explanation}</p>
...
const ReportDisplay: React.FC<{ item: ReportItem, onToggleSelection: (evidenceId: string) => void, isSelected: boolean }> = ({ item, onToggleSelection, isSelected }) => {
    ...
    const isContradictionReport = report.contradictions.length > 0 && report.findings.length === 0;
    ...
    return (
        <div className="mt-1">
            ...
            <div className={`grid grid-cols-1 ${hasImageEvidence ? 'xl:grid-cols-2' : ''} gap-6 bg-gray-800/50 p-4 rounded-lg border border-gray-600 mt-2`}>
                ...
                <div className={`prose prose-invert prose-sm max-h-[70vh] overflow-y-auto pr-2 ${!hasImageEvidence ? 'col-span-1' : ''}`}>
                    ...
                    {report.contradictions.length > 0 && <h4 className="font-bold text-base text-gray-100 border-b border-gray-600 pb-2 my-3">Contradictions Detected</h4>}
                    <div className="space-y-3">
                        {report.contradictions.map((c, index) => <ContradictionDisplay key={index} contradiction={c} />)}
                    </div>
                </div>
            </div>
            {!isContradictionReport && <PdfExportButton report={report} evidence={evidence} />}
            ...
            {!isActionTaken && (
                <div className="flex space-x-2 mt-3">
                    {item.actions.map(action => (
                        <button key={action.label} onClick={() => handleAction(action.callback)} className="px-3 py-1.5 text-sm font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 transition">
                            {action.label}
                        </button>
                    ))}
                </div>
...
const ChatMessage: React.FC<{ item: ChatItem; onToggleSelection: (id: string) => void; isSelected: boolean; }> = ({ item, onToggleSelection, isSelected }) => {
    ...
    return (
        <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} message-bubble`}>
            <div className={`p-4 rounded-2xl w-full max-w-lg lg:max-w-4xl ${isUser ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-700 text-gray-200 rounded-bl-none'}`}>
